...
import { KenyaHotspotMap } from '@/components/dashboard/KenyaHotspotMap';
...
type DashboardView = 'briefing' | 'analyst';

export default function DashboardPage() {
  const { t } = useLanguage();
  const [view, setView] = useState<DashboardView>('briefing');
    ...
    <div className="p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-1 p-1 bg-slate-100 rounded-lg mb-6 w-fit">
          {([
            { id: 'briefing' as const, label: 'Briefing Room' },
            { id: 'analyst' as const, label: 'Analyst View' },
          ]).map((tab) => (
            <button
              key={tab.id}
              type="button"
        ...
        {view === 'briefing' && (
          <div className="space-y-6">
            ...
            <div className={chartCardClass}>
              <h2 className={chartTitleClass}>Hotspot Map</h2>
              <p className={chartDescClass}>
                Geographic view of harmful content across Kenya — counties colored by risk level.
              </p>
              <KenyaHotspotMap />
            </div>

            {/* 4. Top 3 Active Threats */}
            <div className={chartCardClass}>
              <h2 className={chartTitleClass}>Active Threats</h2>
              <p className={chartDescClass}>
                The most significant harmful narratives currently circulating, ranked by severity.
              </p>
              <TopThreatsCard />
            </div>
            ...
            <div className={chartCardClass}>
              <h2 className={chartTitleClass}>Coordinated Activity</h2>
              <p className={chartDescClass}>
                Groups of accounts pushing the same message across multiple counties — potential organized disinformation.
              </p>
              <CoordinatedCampaignsWidget />
            </div>

            {/* 6. Trend: Better or Worse? */}
            <TrendIndicator />

            {/* 7. Recent Escalations */}
            <div className={chartCardClass}>
              <h2 className={chartTitleClass}>Recent Escalations</h2>
              <p className={chartDescClass}>
                Reports that have been flagged for urgent attention by moderators.
              </p>
              <RecentEscalations />
            </div>
          </div>
