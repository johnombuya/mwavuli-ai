  ...
  return (
    <div>
      ...
      <ul className="space-y-2 max-h-64 overflow-y-auto">
        {data.reports.map((r: { id: string; text?: string; risk_level?: string; status?: string; timestamp?: string }) => (
          <li key={r.id} className="bg-white border border-gray-200 rounded p-3 text-sm">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <span className={RISK_COLORS[r.risk_level as string] || 'text-gray-600'}>{r.risk_level || '—'}</span>
              <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${STATUS_COLORS[r.status as string] || 'bg-gray-100 text-gray-700'}`}>
                {r.status || 'pending'}
              </span>
            </div>
          </li>
